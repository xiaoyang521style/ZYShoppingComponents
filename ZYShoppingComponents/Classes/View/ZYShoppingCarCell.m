...
@end
@implementation ZYShoppingCarCell
...
- (void)initCellView{
    ...
    UIImage *btimg = [UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"ic_cb_normal.png"]];
    UIImage *selectImg = [UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"ic_cb_checked.png"]];
    ...
    _sizeLab.textColor=[UIColor colorWithHexString:@"666666"];
    ...
    _soldoutLab.textColor=[UIColor colorWithHexString:@"666666"];
...
- (void)setModel:(ZYShoppingCarModel *)model
{
    _model = model;
    ...
    _shoppingImgView.layer.borderColor = [UIColor colorWithHexString:@"e2e2e2"].CGColor;
    NSString  *bundlePath = [[NSBundle mainBundle] pathForResource:@"Frameworks/ZYShoppingComponents.framework/ZYShoppingComponents" ofType:@"bundle"];
 
    [_shoppingImgView sd_setImageWithURL:[NSURL URLWithString:model.item_info.icon] placeholderImage:[UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"default"]]];
    
    if ([model.item_info.is_spu intValue] == 1) {
        UIImage *img = [UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"spuIcon"]];
        _spuImgView.image = img;
      //  _spuImgView.size = img.size;
        _priceLab.text=[NSString stringWithFormat:@"套装价￥%@",model.item_info.sale_price];
    } else {
        
        UIImage *level = [UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"level2"]];
        if (model.item_info.type == 5) {
            _spuImgView.image = [UIImage imageWithContentsOfFile:[bundlePath stringByAppendingPathComponent:@"level3"]];
    ...
    if ([model.item_info.sale_state isEqualToString:@"3"]) {
        ...
        _selectBt.enabled=YES;
        _changeView = [[ZYChangeCountView alloc] initWithFrame:CGRectMake(_title.frame.origin.x, CGRectGetMaxY(_sizeLab.frame)+5, 160, 35) chooseCount:self.choosedCount totalCount: [model.item_info.stock_quantity integerValue]];
